	...
	private IEntity fillJsonArray (JsonArray jarr,AbstractCollectionEntity<IEntity> entity){
		...
		for( JsonElement jelm:jarr ){
			if( jelm.isJsonObject() ){
				IEntity subentity = fillJsonObject( jelm.getAsJsonObject() , this.config.getFactory().getEntity( entity.getSubTagName() , this.config));
				invokemethod(entity, entity.getSubTagName() , subentity);
			} else if( jelm.isJsonArray() ){
				IEntity subentity = fillJsonArray( jelm.getAsJsonArray() , (AbstractCollectionEntity<IEntity>)this.config.getFactory().getEntity( entity.getSubTagName() , this.config));
				invokemethod(entity, entity.getSubTagName() , subentity);
	...
	private IEntity fillJsonObject (JsonObject jobj,IEntity entity) {
		...
		for(Entry<String, JsonElement> elem:jobj.entrySet()){
			if(elem.getValue().isJsonObject()){
				IEntity subentity = fillJsonObject(elem.getValue().getAsJsonObject(), this.config.getFactory().getEntity(elem.getKey(), this.config));
				invokemethod(entity, elem.getKey(), subentity);
			} else if( elem.getValue().isJsonArray() ){
				IEntity subentity = fillJsonArray( elem.getValue().getAsJsonArray() , (AbstractCollectionEntity<IEntity>)this.config.getFactory().getEntity( elem.getKey() , this.config));
				invokemethod(entity, elem.getKey() , subentity);
